...
def main(argv):
    ...
    MOM = ['MomentumX','momx','mom','|m_i|']
    DEN = ['Density','dens','rho']
    MA = ['Ma','M']
    PRES = ['P','pres','PF']
    EINT = ['e','eint','eF']
    VEL = ['u','velx','uF','V','|v_i|']
    ...
    #group = [DEN, CSP, rhoE, MOMX]
    group = [DEN, MOM, rhoE, MA]
    ...
    for o, a in opts:
        if o in ("-h", "-help"):
            print( 'CVmultiXY.py -i <inputfile> -t <tab separated data file> -o <outputfile>')
            sys.exit(0)
        elif o in ("-i", "-infile"):
            ...
            for idx, gr in enumerate(group, start=0):
                ...
                for  id, n in enumerate(gr, start=0):
                    #print("first n: ", n, " gr: ", gr[id])
                    if n in d:
                        #print(n, d[n],"\n")
                        ax[I,J].plot(d[names[0]], d[n], label=gr[id])
                        ax[I,J].set_ylabel(n)
