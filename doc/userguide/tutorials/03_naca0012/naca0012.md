...
Regarding actual evaluation, we want to calculate the temporal average and the fluctuations, simply calculated as $U'=U-\bar{U}$. You can run the tool using

~~~~~~~
posti_visualizerecordpoints parameter_visualizeRecordpoints.ini NACA0012_Re5000_AoA4_RP_*
~~~~~~~

This will take all the time samples recorded during our simulation as input. For each of the planes, a seperate *.vts* file contaning the temporal average will be written. The fluctuations are time-resolved data, and due to limitations in the VTK file format
