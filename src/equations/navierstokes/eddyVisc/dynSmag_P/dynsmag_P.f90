...
! Public Part ----------------------------------------------------------------------------------------------------------------------
INTERFACE InitDynsmag_P
  MODULE PROCEDURE InitDynsmag_P
END INTERFACE

INTERFACE Dynsmag_P
  MODULE PROCEDURE Dynsmag_P
END INTERFACE

INTERFACE Compute_Cd_P
  MODULE PROCEDURE Compute_Cd_P
END INTERFACE

INTERFACE FinalizeDynsmag_P
  MODULE PROCEDURE FinalizeDynsmag_P
END INTERFACE

PUBLIC::InitDynsmag_P,Dynsmag_P,Compute_Cd_P,FinalizeDynsmag_P
!===================================================================================================================================
...
SWRITE(UNIT_stdOut,'(A)')' INIT Dynsmag P DONE!'
SWRITE(UNIT_StdOut,'(132("-"))')
END SUBROUTINE InitDynsmag_P

!===================================================================================================================================
!> Compute Dynsmag Eddy-Visosity at a given volume point
!===================================================================================================================================
SUBROUTINE Dynsmag_P(iElem,i,j,k,muSGS)
! MODULES
USE MOD_PreProc
...
REAL,DIMENSION(3,3,0:PP_N,0:PP_N,0:PP_N)     :: gradv_all, S_ik, Q_ik, L_ik, P_ik
REAL,DIMENSION(3,0:PP_N,0:PP_N,0:PP_N)       :: V_filtered,V_In
REAL, DIMENSION(0:PP_N,0:PP_N,0:PP_N)        :: QQ,QLP,S_eN_filtered,S_eN,divV
REAL                                         :: D_Ratio
REAL                                         :: QQa,QLPa
!===============================================================================================================================
DO iElem=1,nElems
...
!  CALL gradient(3,V_In,gradv_all(:,3,:,:,:),iElem)
  ...
  !            _____            _ _   ____
  !Compute P=filt( Cd Delta^2 |S|S)  !!TENSOR ik
  DO i=1,3
    DO k=1,3
      P_ik(i,k,:,:,:) = S_eN(:,:,:)*S_ik(i,k,:,:,:)*SGS_Ind(1,:,:,:,iElem)
    END DO ! k
    CALL Filter_Selective(3,FilterMat_testfilter,P_ik(i,1:3,:,:,:),filter_ind(:,iElem))
  END DO ! i
...
!  !get the gradients of filtered variables
...
!  D_Ratio =  (REAL(PP_N)/REAL(N_testfilter))**(2./3.)
  ...
  !Compute Q = (Delta_test/Delta)^2 filt(|S|) filt(S_ik)
  END SELECT
  DO i=1,3
    DO k=1,3
      Q_ik(i,k,:,:,:) =  D_Ratio * S_eN_filtered(:,:,:)*S_ik(i,k,:,:,:)
    END DO ! k
  END DO ! i
  !
  !Compute C_d Delta^2 = Q_ik*(L_ik-P_ik)/Q_ik*Q_ik (doubts on the 1/2)
  QLP=0.
  QQ=0.
  DO i=0,PP_N
    DO j=0,PP_N
      DO k=0,PP_N
        DO l=1,3
          DO m=1,3
            QQ(i,j,k)  = QQ(i,j,k) + (Q_ik(l,m,i,j,k)*Q_ik(l,m,i,j,k))
            QLP(i,j,k)  = QLP(i,j,k) + Q_ik(l,m,i,j,k)*(L_ik(l,m,i,j,k)-P_ik(l,m,i,j,k))
          END DO ! k
        END DO ! j
      END DO ! k
    END DO ! j
...
!  SGS_Ind(1,:,:,:,iElem) = ML(:,:,:)/MM(:,:,:)
  ...
  CASE (1) ! Volume
    QQa = 0.; QLPa = 0.
    DO i=0,PP_N; DO j=0,PP_N; DO k=0,PP_N
      QQa = QQa + QQ(i,j,k)*IntElem(i,j,k,iElem)
      QLPa = QLPa + QLP(i,j,k)*IntElem(i,j,k,iElem)
    END DO; END DO; END DO !ijk
    SGS_Ind(1,:,:,:,iElem) = 0.5*QLPa/QQa
  CASE (2) ! I-Plane
    DO i=0,PP_N
      QQa = 0.; QLPa = 0.
      DO j=0,PP_N; DO k=0,PP_N
        ...
        QLPa = QLPa + QLP(i,j,k)*IntElem(i,j,k,iElem)
      END DO; END DO !jk
      SGS_Ind(1,i,:,:,iElem) = 0.5*QLPa/QQa
    END DO !i
  CASE (3) ! J-Plane
    DO j=0,PP_N
      QQa = 0.; QLPa = 0.
      DO i=0,PP_N; DO k=0,PP_N
        ...
        QLPa = QLPa + QLP(i,j,k)*IntElem(i,j,k,iElem)
      END DO; END DO !jk
      SGS_Ind(1,:,j,:,iElem) = 0.5*QLPa/QQa
    END DO !j
  CASE (4) ! K-Plane
    DO k=0,PP_N
      QQa = 0.; QLPa = 0.
      DO i=0,PP_N; DO j=0,PP_N
        ...
        QLPa = QLPa + QLP(i,j,k)*IntElem(i,j,k,iElem)
      END DO; END DO !jk
      SGS_Ind(1,:,:,k,iElem) = 0.5*QLPa/QQa
    END DO !k
  CASE (5) ! I-Line
    DO j=0,PP_N; DO k=0,PP_N
      QQa = 0.; QLPa = 0.
      DO i=0,PP_N
        ...
        QLPa = QLPa + QLP(i,j,k)*IntElem(i,j,k,iElem)
      END DO ! i
      SGS_Ind(1,:,j,k,iElem) = 0.5*QLPa/QQa
    END DO; END DO !jk
  CASE (6) ! J-Line
    DO i=0,PP_N; DO k=0,PP_N
      QQa = 0.; QLPa = 0.
      DO j=0,PP_N
        ...
        QLPa = QLPa + QLP(i,j,k)*IntElem(i,j,k,iElem)
      END DO ! i
      SGS_Ind(1,i,:,k,iElem) = 0.5*QLPa/QQa
    END DO; END DO !jk
  CASE (7) ! K-Line
    DO i=0,PP_N; DO j=0,PP_N
      QQa = 0.; QLPa = 0.
      DO k=0,PP_N
        ...
        QLPa = QLPa + QLP(i,j,k)*IntElem(i,j,k,iElem)
      END DO ! i
      SGS_Ind(1,i,j,:,iElem) = 0.5*QLPa/QQa
    END DO; END DO!jk
  END SELECT
END DO

!MATTEO: DEBUG testing forcing a precise value to test output
SGS_Ind(1,:,:,:,:)=0.1

END SUBROUTINE Compute_Cd_P
...
END SUBROUTINE FinalizeDynsmag_P

#endif
END MODULE MOD_dynsmag_P
