...
DO iElem=1,nElems
  ...
  DO k=0,PP_NZ
    DO j=0,PP_N
      DO i=0,PP_N
        Lambda1=ABS(SUM(Metrics_fTilde(:,i,j,k,iElem,FVE)*U(:,i,j,k,iElem)))
        Lambda2=ABS(SUM(Metrics_gTilde(:,i,j,k,iElem,FVE)*U(:,i,j,k,iElem)))
#if PP_dim==3
        Lambda3=ABS(SUM(Metrics_hTilde(:,i,j,k,iElem,FVE)*U(:,i,j,k,iElem)))
